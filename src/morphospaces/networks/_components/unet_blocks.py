"""Adapted from pytorch3dunet.unet3d.building_blocks
https://github.com/wolny/pytorch-3dunet

MIT License
For original license see LICENSE file provided with the original source code.
"""
...
def create_conv(
    ...
    """
    Create a list of modules with together constitute a single conv layer
    with non-linearity and optional batchnorm/groupnorm.
    ...
    Args:
        ...
        padding (int or tuple): add zero-padding added to all three sides
            of the input
    Return:
    ...
    for i, char in enumerate(order):
        if char == "r":
            ...
            assert num_channels % num_groups == 0, (
                f"Expected number of channels in input to "
                f"be divisible by num_groups. "
            ...
            raise ValueError(
                f"Unsupported layer type '{char}'. MUST be one of"
                f"['b', 'g', 'r', 'l', 'e', 'c']"
            )
...
class DoubleConv(nn.Sequential):
    ...
    This can be changed however by providing the 'order' argument,
    e.g. in order to change to Conv3d+BatchNorm3d+ELU use
    order='cbe'. Use padded convolutions to make sure that
    the output (H_out, W_out) is the same
    as (H_in, W_in), so that you don't have to crop in the
    decoder path.
    ...
    Args:
        ...
        order (string): determines the order of layers, e.g.
            ...
            'ce' -> conv + ELU
        num_groups (int): number of groups for the GroupNorm
        padding (int or tuple): add zero-padding added to
            all three sides of the input
    """
...
class ResNetBlock(nn.Module):
    """
    Residual block that can be used instead of standard DoubleConv
    in the Encoder module.
    Motivated by: https://arxiv.org/pdf/1706.00120.pdf

    Notice we use ELU instead of ReLU (order='cge') and put
    non-linearity after the groupnorm.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # remove non-linearity from the 3rd convolution since it's
        # going to be applied after adding the residual
        n_order = order
        for c in "rel":
    ...
    def forward(self, x):
        # apply first convolution to bring the number of channels
        # to out_channels
...
class Encoder(nn.Module):
    ...
    (make sure to use complementary scale_factor in the decoder path)
    ...
    Args:
        ...
        num_groups (int): number of groups for the GroupNorm
        padding (int or tuple): add zero-padding added to all
...
class Decoder(nn.Module):
    ...
    Args:
        ...
        scale_factor (tuple): used as the multiplier for the image H/W/D in
            ...
            must reverse the MaxPool3d operation
    ...
    def __init__(
        ...
        if upsample:
            if basic_module == DoubleConv:
                ...
                # if basic_module=ResNetBlock use transposed convolution
...
def create_encoders(
    ...
    # Depth of the encoder is equal to `len(f_maps)`
    encoders = []
...
class AbstractUpsampling(nn.Module):
    """
    Abstract class for upsampling. A given implementation should upsample
    a given 5D input tensor using either interpolation or
...
class TransposeConvUpsampling(AbstractUpsampling):
    ...
    def __init__(
        ...
        # make sure that the output size reverses
        # the MaxPool3d from the corresponding encoder
